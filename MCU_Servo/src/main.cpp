...
void setup()
{
    ...
    WIFI_OP_init();
    WIFI_OP_MQTT_init();
    servo_state.step(); // After initialization, step to SERVO_READY
}
...
void loop()
{
    ...
    servo_state.step(); // After receiving message, step to SERVO_RECEIVE_COMMAND
    ...
    servo_state.step(); // After moving servo, step to SERVO_MOVE
    ...
    servo_state.step(); // After returning home, step to SERVO_RETURN_HOME
    // TODO: Check servo position before stepping to SERVO_READY, if not in expected position, set error state
    servo_state.step(); // After checking position, step to SERVO_READY
    u8_Message_flag = 0;
}
